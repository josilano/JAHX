...
@section('navigation')
    ...
    <div class="col l12 m12 s12 black">
        <h3 class="yellow-text text-accent-4 center-align">CATEGORIAS</h3>
        <table class="highlight yellow-text text-accent-4 container">
            ...
            <tbody>
                <tr>
                    ...
                    <td>@if(isset($categorias)){{ $categorias->total() }}@endif</td>
                </tr>
            </tbody>
        </table>
    </div>
...
@section('content')
    ...
    <ul class="collapsible" data-collapsible="accordion">
        <li>
            <div class="collapsible-header yellow accent-4"><i class="material-icons">filter_drama</i>Cadastrar Categoria</div>
            <div class="collapsible-body black">
                <form id="form-create-categoria" action="{{ route('jahx.addcategoria') }}" method="post">
                {{ csrf_field() }}
                <input type="hidden" id="user-id" name="user-id" value="{{ Auth::user()->id }}">

                <fieldset style="border-radius: 20px; border-color: #ffd600;">
                    <legend class="purple-text text-darken-3 yellow-text text-accent-4">Dados da Categoria</legend>
                    <div class="row">
                        <div class="input-field col s12 m12 l12 yellow-text text-accent-4">
                            ...
                            <input required type="text" id="nome-categoria" name="nome-categoria" maxlength="60" data-length="60">
                            <label for="nome-categoria">Nome da Categoria</label>
                        </div>
                    </div>
                </fieldset>
                <br>
                <button class="btn waves-effect waves-light yellow accent-4 black-text" type="submit" name="action">Cadastrar Categoria
    ...
    @if(isset($categorias))
                ...
                <div class="collapsible-body">
                    ...
                    <div class="row">
                        <div class="col l12 m12 s12 center-align yellow lighten-5">
                            ...
                            <a class="" href="{{ route('jahx.mostracategoria', ['id' => $categoria->id]) }}">Editar</a>
                        </div>
                    </div>
                </div>
            </li>
        ...
        @include('erp.partials.componentepaginacao', ['paginacao' => $categorias])
...
@section('rodape')
    ...
    @if(isset($cadcategoriaexception))
        @include('erp.partials.msgexception', ['exception' => $cadcategoriaexception])
    @endif
@endsection
...
@section('script')
    ...
    @if(isset($cadcategoria))
        @if(!is_numeric($cadcategoria))
        <script type="text/javascript">
            msgtela('Categoria cadastrada');
        </script>
        @endif
    @else
        <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            onAutocomplete: function(slug) {
                ...
                $.ajax({
                    ...
                    url: "/erp/ajustes/categorias/q="+slug,
                    type: 'GET',
                    //data: {slug},
                    headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                    success: function (response) {
                        window.location.replace("/erp/ajustes/categorias/q="+slug);
                        debugger;
                        console.log(slug);
                    }
                });
