...
@section('navigation')
    ...
    <div class="col l12 m12 s12 black">
        <h3 class="yellow-text text-accent-4 center-align">EDITAR UNIDADE DE MEDIDA</h3>
    </div>
@endsection
...
@section('content')
    ...
    <div class="col l3 m3 s4 yellow-text text-accent-4">
        <a href="{{ route('jahx.unmedidas') }}" class="yellow-text text-accent-4 valign-wrapper">
    ...
    @if(isset($unmedida))
        <div class="col l12 m12 s12">
            <form id="form-update-marca" action="{{ route('jahx.atualizaunmedida', ['id' => $unmedida->id]) }}" method="post">
                {{ method_field('PUT') }}
                {{ csrf_field() }}
                ...
                <legend class="yellow accent-4">Editar Unidade de Medida</legend>
                ...
                <div class="row">
                    <div class="input-field col s12 m12 l12 yellow-text text-accent-4">
                            ...
                            <input required type="text" id="sigla" name="sigla" maxlength="60" data-length="60" value="{{ $unmedida->sigla }}">
                            <label for="sigla">Sigla</label>
                        </div>
                        <div class="input-field col s12 m12 l12 yellow-text text-accent-4">
                            <i class="material-icons yellow-text text-accent-4 prefix">loyalty</i>
                            <input required type="text" id="descricao" name="descricao" maxlength="60" data-length="60" value="{{ $unmedida->descricao }}">
                            <label for="descricao">Descrição</label>
                    </div>
                </div>
                </fieldset>
                <br>
                <button class="btn waves-effect waves-light yellow accent-4 black-text" type="submit" name="action">Atualizar Unidade de Medida
...
@section('rodape')
    ...
    @if(isset($upunmedidaexception))
        @include('erp.partials.msgexception', ['exception' => $upunmedidaexception])
    @endif
@endsection
...
@section('script')
    ...
    @if(isset($upunmedida))
        @if(!is_numeric($upunmedida))
        <script type="text/javascript">
            msgtela('Unidade de Medida atualizada');
        </script>
        @endif
    @else
        <script type="text/javascript">
