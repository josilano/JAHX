...
@section('navigation')
    ...
    <div class="col l12 m12 s12 black">
        <h3 class="yellow-text text-accent-4 center-align">PRÉ-VENDAS</h3>
        <table class="highlight yellow-text text-accent-4 container">
            ...
            <tbody>
                <tr>
                    <td>Total Pré-Vendas Realizadas</td>
                    <td>@if(isset($davs)){{ $davs->total() }}@endif</td>
                </tr>
            </tbody>
        </table>
    </div>
...
@section('content')
    ...
    <div class="row">
        <div class="col l6 m6 s12">
            <a class="waves-effect waves-light btn yellow accent-4 black-text" href="{{ route('jahx.criardav') }}"><i class="material-icons left black-text">flag</i>Iniciar pré-venda (DAV)</a>
        </div>
    ...
    @if (isset($davs))
        @include ('erp.partials.davs.componentelistadavs', ['vendas' => $davs])

        @include ('erp.partials.componentepaginacao', ['paginacao' => $davs])
    @endif
    
@endsection

@section('rodape')
    @if(isset($davsexception))
        @include('erp.partials.msgexception', ['exception' => $davsexception])
    @endif

    @if(isset($caddavexception))
        @include('erp.partials.msgexception', ['exception' => $caddavexception])
    @endif
@endsection
...
@section('script')
    ...
    @if(is_null($davs))
        <script type="text/javascript">
            msgtela('sem cadastro de pré-vendas');
        </script>
    @endif
    
    @if(isset($caddav))
        @if(!is_numeric($caddav))
        <script type="text/javascript">
            msgtela('Pré-Venda cadastrada');
        </script>
        @endif
    @else
        <script type="text/javascript">
    ...
    <script type="text/javascript">
            ...
            onAutocomplete: function(slug) {
                $.ajax({
                    ...
                    success: function (response) {
                        window.location.replace("/erp/davs/"+slug);
                        debugger;
                        console.log(slug);
                    }
                });
        ...
        function buscaDavPorData(){
            ...
            window.location.replace("/erp/davs/periodo/venda/"+dia);
        }
    </script>
